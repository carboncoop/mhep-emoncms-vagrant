...
def main():
    ...
    got_assessment = bot.export_assessment()
    with open('test_data/got_assessment.json', 'w') as f:
...
class MHEPPopulateBot(MHEPBaseBot):
    def __init__(self, webdriver, test_assessment):
        ...
        self.assessment = test_assessment
...
def diff_assessments(left, right):
    def sort(od):
        return sorted(od.items(), key=lambda kv: kv[0].upper())

    left_flattened = sort(flatten_json(left))
    right_flattened = sort(flatten_json(right))

    from jsondiff import diff
    return diff(left_flattened, right_flattened)
